...
// See also https://developer.mozilla.org/en/JavaScript/Reference/Global-Objects/Error/Stack
 ...
 * (https://code.google.com/p/google-diff-match-patch/source/browse/trunk/javascript/diff-match-patch-uncompressed.js),
...
QUnit.diff = ( function() {
	...
	DiffMatchPatch.prototype.DiffMain = function( text1, text2, optChecklines ) {
		...
		if ( commonprefix ) {
			diffs.unshift( [ DIFF-EQUAL, commonprefix ] );
		}
		if ( commonsuffix ) {
			diffs.push( [ DIFF-EQUAL, commonsuffix ] );
		}
	...
	DiffMatchPatch.prototype.diffCleanupEfficiency = function( diffs ) {
		...
		while ( pointer < diffs.length ) {
			...
			// An insertion or deletion.
				...
				if ( diffs[ pointer ][ 0 ] === DIFF-DELETE ) {
					postDel = true;
				} else {
					postIns = true;
				}
				...
				if ( lastequality && ( ( preIns && preDel && postIns && postDel ) ||
					...
					diffs[ equalities[ equalitiesLength - 1 ] + 1 ][ 0 ] = DIFF-INSERT;
	...
	DiffMatchPatch.prototype.diffCompute = function( text1, text2, checklines, deadline ) {
		...
		if ( i !== -1 ) {
			...
			if ( text1.length > text2.length ) {
				diffs[ 0 ][ 0 ] = diffs[ 2 ][ 0 ] = DIFF-DELETE;
			}
			return diffs;
		}
	...
	DiffMatchPatch.prototype.diffCleanupSemantic = function( diffs ) {
		...
		lengthDeletions2 = 0;
		while ( pointer < diffs.length ) {
			if ( diffs[ pointer ][ 0 ] === DIFF-EQUAL ) { // Equality found.
				equalities[ equalitiesLength++ ] = pointer;
				...
				if ( diffs[ pointer ][ 0 ] === DIFF-INSERT ) {
		...
		pointer = 1;
		while ( pointer < diffs.length ) {
			if ( diffs[ pointer - 1 ][ 0 ] === DIFF-DELETE &&
					diffs[ pointer ][ 0 ] === DIFF-INSERT ) {
				...
				if ( overlapLength1 >= overlapLength2 ) {
					...
					if ( overlapLength2 >= deletion.length / 2 ||
						...
						diffs[ pointer - 1 ][ 0 ] = DIFF-INSERT;
						diffs[ pointer - 1 ][ 1 ] =
							insertion.substring( 0, insertion.length - overlapLength2 );
						diffs[ pointer + 1 ][ 0 ] = DIFF-DELETE;
						diffs[ pointer + 1 ][ 1 ] =
	...
	DiffMatchPatch.prototype.diffCleanupMerge = function( diffs ) {
		...
		diffs.push( [ DIFF-EQUAL, "" ] ); // Add a dummy entry at the end.
		pointer = 0;
		countDelete = 0;
		countInsert = 0;
		textDelete = "";
				...
				if ( countDelete + countInsert > 1 ) {
					...
					if ( countDelete === 0 ) {
						diffs.splice( pointer - countInsert,
							countDelete + countInsert, [ DIFF-INSERT, textInsert ] );
					} else if ( countInsert === 0 ) {
						diffs.splice( pointer - countDelete,
							countDelete + countInsert, [ DIFF-DELETE, textDelete ] );
					} else {
						diffs.splice(
							pointer - countDelete - countInsert,
							countDelete + countInsert,
							[ DIFF-DELETE, textDelete ], [ DIFF-INSERT, textInsert ]
						);
					}
					pointer = pointer - countDelete - countInsert +
						( countDelete ? 1 : 0 ) + ( countInsert ? 1 : 0 ) + 1;
				} else if ( pointer !== 0 && diffs[ pointer - 1 ][ 0 ] === DIFF-EQUAL ) {
		...
		while ( pointer < diffs.length - 1 ) {
			if ( diffs[ pointer - 1 ][ 0 ] === DIFF-EQUAL &&
					diffs[ pointer + 1 ][ 0 ] === DIFF-EQUAL ) {
